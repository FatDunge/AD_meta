...
do
    ...
    do
        if [[ $filename =~ $brain ]]
        then
            flirt -in ${my_path}${center}${filename} -ref /usr/local/fsl/data/standard/MNI152_T1_1mm_brain.nii.gz -out ${my_path}${center}${filename}_flirt -bins 256 -cost corratio -searchrx -90 90 -searchry -90 90 -searchrz -90 90 -dof 12  -interp trilinear
